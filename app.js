...
app.post('/api/login-help', userAnalytics)
app.listen(port)
...
function userAnalytics(req, res) {
    var username = req.body.username;
    var userStats = JSON.parse(req.body.userData);
    //Use FS to first read the userData then it writes all of our analytics to it.

    fileSystem.readFile('userData.json', (err, content) => {
        if (err) throw err;
